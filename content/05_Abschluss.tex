...
Diese Arbeit untersuchte die Entwicklung und Evaluation\index{Evaluation!Agenten} agentischer Architekturen\index{Referenzarchitektur} für Software-Engineering-Workflows\index{SE-Workflows}.\footnote{Vergleiche dazu \cite{lee2018conference} und \cite{website2021example} für aktuelle Forschungstrends.} Ausgehend von einer systematischen Analyse des Stands der Forschung (Kapitel \ref{ch:hintergrund}) wurde eine Referenzarchitektur entwickelt\index{Architekturdesign} (Kapitel \ref{ch:konzept}), prototypisch implementiert und empirisch evaluiert\index{Empirische Evaluation} (Kapitel \ref{ch:realisierung}).
...
\begin{itemize}
  \item Explizite Thought-Action-Observation-Loops\index{Thought-Action-Observation} für Transparenz\index{Agent-Transparenz}
  \item Strukturierte Tool-Interfaces\index{Tool-Interfaces!Strukturierung} mit JSON-Schema-Validation\index{Schema-Validation}
  \item Episodisches Gedächtnis\index{Episodisches Gedächtnis!Design} für Kontext-Persistierung\index{Kontext-Persistierung} über Iterationen
\end{itemize}
...
Die entwickelte Architektur implementiert Defense-in-Depth\index{Defense-in-Depth} durch mehrschichtige Sicherheitsmechanismen:
\begin{enumerate}
  \item Input-Validation\index{Input-Validation!Tool-Calls} aller LLM-generierten Tool-Calls
  \item Sandbox-Execution\index{Sandbox!Docker} in isolierten Docker-Containern
  \item Permissions-System\index{Permissions-System} mit Least-Privilege-Prinzip\index{Least-Privilege}
  \item Audit-Logging\index{Audit-Logging!Tool-Calls} aller kritischen Operationen
  \item Human-in-the-Loop\index{Human-in-the-Loop} für Deployment-kritische Aktionen
\end{enumerate}
