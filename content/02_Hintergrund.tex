...
Dieses Kapitel führt in Grundbegriffe agentischer Systeme ein und bildet die theoretische Basis für Konzept und Implementierung.\footnote{Die Grundbegriffe basieren auf etablierten Konzepten aus der KI-Forschung, werden aber im Kontext von Software Engineering neu interpretiert.}
...
Moderne LLMs wie GPT-4, Claude oder Llama 2 verfügen über Milliarden von Parametern und können durch \emph{Few-Shot-Learning}\index{Few-Shot-Learning} und \emph{In-Context-Learning}\index{In-Context-Learning} Aufgaben lösen, ohne explizit darauf trainiert zu werden. Dies bildet die Grundlage für agentische Anwendungen\index{Agentische Anwendungen}.

\subsection{Von LLMs zu Agenten: Der Paradigmenwechsel}

Während klassische LLMs primär auf Textvervollständigung spezialisiert sind, zeichnen sich \emph{agentische Systeme} durch folgende erweiterte Fähigkeiten aus \cite{wang2023survey}:

\begin{enumerate}
  \item \textbf{Zielgerichtetes Handeln:} Der Agent\index{Agent!Zielgerichtetheit} versteht übergeordnete Ziele und plant Schritte zur Zielerreichung\index{Zielerreichung}
  \item \textbf{Werkzeugnutzung:} Integration externer Tools\index{Externe Tools} (APIs, Datenbanken, Code-Execution)\index{Code-Execution}
  \item \textbf{Gedächtnis:} Persistierung von Kontextinformationen\index{Kontextpersistierung} über mehrere Interaktionen hinweg
  \item \textbf{Reflexion:} Selbstkritische Bewertung\index{Selbstbewertung} eigener Outputs und iterative Verbesserung
  \item \textbf{Mehrschrittige Planung:} Dekomposition\index{Task-Dekomposition} komplexer Aufgaben in Teilschritte
\end{enumerate}

\subsection{Agentic AI: Begriffe und Bausteine}

Kernbausteine agentischer Systeme sind \cite{smith2020example,yao2023react}:

\begin{description}
  \item[Zustand (State):] Umfasst aktuellen Kontext\index{State Management}, Ziele, Erinnerungen und Tool-Status\index{Tool-Status}. Der Zustand wird dynamisch aktualisiert\index{State Updates}.
  
  \item[Policy:] Steuert Entscheidungsprozesse (Planung, Aktion, Selbstkritik)\index{Policy!Decision-Making}. Kann regelbasiert\index{Regelbasierte Policy} oder LLM-gesteuert\index{LLM-gesteuerte Policy} sein.
  
  \item[Werkzeuge (Tools):] Externe Funktionen wie Code-Ausfürung, Websuche\index{Websuche}, Dateizugriff, VCS-Operationen\index{VCS-Operationen}. Tools erweitern die Fähigkeiten des Agenten über reine Textgenerierung hinaus \cite{schick2023toolformer,qin2023tool}.
...
Praxisnahe Frameworks bieten Abstraktionen für agentische Systeme \cite{doe2019research}:\footnote{\emph{ReAct} steht für \enquote{Reasoning and Acting} und ist eines der einflussreichsten Muster für agentische Systeme in der neueren Literatur.}
...
\textquote[\cite{doe2019research}]{Die Kombination von reasoning und acting führt zu robusteren und transparenteren Systemen, die besser nachvollziehbare Entscheidungen treffen.}
...
\textbf{Reflexion} \cite{shinn2023reflexion} erweitert ReAct um Self-Critique: Der Agent bewertet seine eigenen Outputs und lernt aus Fehlern durch verbal reinforcement. Dies verbessert die Erfolgsrate bei komplexen Tasks um bis zu \pct{20}.
...
\begin{itemize}
  \item Spezialisierte Tools für Navigation, Suche und Editing
  \item Kontextoptimierte Feedback-Formate
  \item Iterative Refinement-Loops
\end{itemize}

\textbf{Agentless} \cite{zhang2024agentless} verfolgte einen minimalistischen Ansatz ohne persistentes Gedächtnis oder komplexe Planung und erreichte dennoch \pct{27} Erfolgsrate durch fokussierte Lokalisierung und Patching-Strategien. Dies zeigt, dass nicht immer maximale Komplexität optimal ist.

\subsection{Multi-Agent-Systeme}

Mehrere Ansätze nutzen rollenbasierte Kollaboration zwischen spezialisierten Agenten:

\textbf{MetaGPT} \cite{hong2023metagpt} simuliert ein Software-Team mit Rollen wie ProductManager, Architect, Engineer und QA. Agents produzieren strukturierte Artefakte (PRDs, Design Docs, Code, Tests) und folgen einem definierten Workflow.
...
Graphen erlauben robuste Kontrollflüsse (Retry, Branching, Parallelisierung), klare Zustandsübergänge und bessere Testbarkeit \cite{lee2018conference}. LangGraph erweitert LangChain um zustandsbasierte Graphen mit deterministischen Übergängen.
...
\begin{enumerate}
  ...
  \item \textbf{Kontextmanagement:} Lange Codebases erfordern effiziente Context-Windows
  \item \textbf{Safety Mechanisms:} Code-Execution erfordert Sandboxing und Validierung
\end{enumerate}
...
\begin{itemize}
  ...
  \item \textbf{Tool-Interface Design:} Unklar ist, wie Tools optimal gestaltet werden sollten (granular vs. high-level, synchron vs. asynchron)
...
\begin{itemize}
  ...
  \item \textbf{Safety und Robustheit:} Wie messen wir Resistenz gegen Prompt-Injection oder malicious Tools?
\end{itemize}

\subsection{Skalierungs- und Kostenfragen}

\begin{itemize}
  \item \textbf{Langer Kontext:} Bei großen Codebases (\textgreater 100k LOC) stoßen selbst 1M-Token-Kontexte an Grenzen
...
\begin{enumerate}
  ...
  \item Praxisnahe Implementierung mit Tool-Interface-Guidelines
